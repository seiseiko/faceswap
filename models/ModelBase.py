...
class ModelBase(object):
    def __init__(self, is_training=False,
                ...
                while True:
                    ...
                    for filepath in pathex.get_file_paths(saved_models_path):
                        ...
                        if filepath_name.endswith(f'{model_class_name}_data.dat'):
                            saved_models_names += [(filepath_name.split('_')
                                                    [0], os.path.getmtime(filepath))]
                    ...
                    if len(saved_models_names) != 0:
                        if silent_start:
                            ...
                            inp = io.input_str(
                                f"", "256_res", show_default_value=False)
                            model_idx = -1
                            try:
                                model_idx = np.clip(
        ...
        ####
        self.default_options_path = saved_models_path / \
            f'{self.model_class_name}_default_options.dat'
        ...
        self.write_preview_history = self.options.get(
    ...
    def ask_autobackup_hour(self, default_value=0):
        default_autobackup_hour = self.options['autobackup_hour'] = self.load_or_def_option(
            'autobackup_hour', default_value)
        self.options['autobackup_hour'] = io.input_int(f"Autobackup every N hour", default_autobackup_hour, add_info="0..24",
    ...
    def ask_write_preview_history(self, default_value=False):
        ...
        self.options['write_preview_history'] = io.input_bool(
            f"Write preview history", default_write_preview_history, help_message="Preview history will be writed to <ModelName>_history folder.")
    ...
    def ask_target_iter(self, default_value=0):
        default_target_iter = self.load_or_def_option(
            'target_iter', default_value)
        self.options['target_iter'] = max(0, io.input_int(
            "Target iteration", default_target_iter))
    ...
    def ask_random_flip(self):
        ...
        self.options['random_flip'] = io.input_bool(
            "Flip faces randomly", default_random_flip, help_message="Predicted face will look more naturally without this option, but src faceset should cover all face directions as dst faceset.")
    ...
    def ask_batch_size(self, suggest_batch_size=None):
        ...
        self.options['batch_size'] = self.batch_size = max(0, io.input_int(
            "Batch_size", default_batch_size, help_message="Larger batch size is better for NN's generalization, but it can cause Out of Memory error. Tune this value for your videocard manually."))
    ...
    def get_static_preview(self):
        ...
        return self.onGetPreview(self.sample_for_preview)[0][1]
    ...
    def create_backup(self):
        ...
        bckp_filename_list = [self.get_strpath_storage_for_file(
            filename) for _, filename in self.get_model_filename_list()]
        bckp_filename_list += [str(self.get_summary_path()),
        ...
        for i in range(24, 0, -1):
            ...
            if i == 1:
                ...
                for preview, filepath in plist:
                    ...
                    img = (np.concatenate(
                        [preview_lh, preview], axis=0) * 255).astype(np.uint8)
    ...
    def get_summary_text(self):
        ...
        width_name = max([len(k) for k in self.options.keys()] + [17]) + 1
        width_value = max([len(str(x)) for x in self.options.values(
        )] + [len(str(self.get_iter())), len(self.get_model_name())]) + 1  # Single space buffer to right edge
        if len(self.device_config.devices) != 0:  # Check length of GPU names
            width_value = max(
                [len(device.name)+1 for device in self.device_config.devices] + [width_value])
