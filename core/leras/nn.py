...
class nn():
            ...
            if len(device_config.devices) != 0:
                if sys.platform[0:3] == 'win':
                    ...
                    if all([x.name == device_config.devices[0].name for x in device_config.devices]):
                        devices_str = "_" + \
                            device_config.devices[0].name.replace(' ', '_')
                    else:
                        devices_str = ""
                    ...
                    compute_cache_path = Path(
                        os.environ['APPDATA']) / 'NVIDIA' / ('ComputeCache' + devices_str)
    ...
    def ask_choose_device_idxs(choose_only_one=False, allow_cpu=True, suggest_best_multi_gpu=False, suggest_all_gpu=False):
        ...
        if suggest_all_gpu:
            ...
            best_device_indexes = [
                device.index for device in devices.get_equal_devices(devices.get_best_device())]
        else:
