...
def MergeMaskedFace(predictor_func, predictor_input_shape,
                    ...
                    cfg, frame_info, img_bgr_uint8, img_bgr, img_face_landmarks, expression_face=None):
    ...
    face_output_mat = LandmarksProcessor.get_transform_mat(
        img_face_landmarks, output_size, face_type=cfg.face_type, scale=1.0 + 0.01*cfg.output_face_scale)
    ...
    if mask_subres_size == output_size:
        ...
        face_mask_output_mat = LandmarksProcessor.get_transform_mat(
            img_face_landmarks, mask_subres_size, face_type=cfg.face_type, scale=1.0 + 0.01*cfg.output_face_scale)

    dst_face_bgr = cv2.warpAffine(
        img_bgr, face_mat, (output_size, output_size), flags=cv2.INTER_CUBIC)
    dst_face_bgr = np.clip(dst_face_bgr, 0, 1)

    dst_face_mask_a_0 = cv2.warpAffine(
        img_face_mask_a, face_mat, (output_size, output_size), flags=cv2.INTER_CUBIC)
    dst_face_mask_a_0 = np.clip(dst_face_mask_a_0, 0, 1)

    # predictor_input_bgr = cv2.resize(dst_face_bgr, (input_size, input_size))
    predictor_input_bgr = cv2.resize(expression_face, (input_size, input_size))
    ...
    if cfg.super_resolution_power != 0:
        prd_face_bgr_enhanced = face_enhancer_func(
            prd_face_bgr, is_tanh=True, preserve_size=False)
        mod = cfg.super_resolution_power / 100.0
        prd_face_bgr = cv2.resize(
            prd_face_bgr, (output_size, output_size))*(1.0-mod) + prd_face_bgr_enhanced*mod
        prd_face_bgr = np.clip(prd_face_bgr, 0, 1)

    if cfg.super_resolution_power != 0:
        prd_face_mask_a_0 = cv2.resize(
            prd_face_mask_a_0,      (output_size, output_size), cv2.INTER_CUBIC)
        prd_face_dst_mask_a_0 = cv2.resize(
            prd_face_dst_mask_a_0,  (output_size, output_size), cv2.INTER_CUBIC)

    if cfg.mask_mode == 1:  # dst
        wrk_face_mask_a_0 = cv2.resize(
            dst_face_mask_a_0, (output_size, output_size), cv2.INTER_CUBIC)
        ...
        if cfg.mask_mode == 6 or cfg.mask_mode == 8 or cfg.mask_mode == 9:
            # obtain XSeg-prd
            prd_face_xseg_bgr = cv2.resize(
                prd_face_bgr, (xseg_input_size,)*2, cv2.INTER_CUBIC)
            prd_face_xseg_mask = xseg_256_extract_func(prd_face_xseg_bgr)
            X_prd_face_mask_a_0 = cv2.resize(
                prd_face_xseg_mask, (output_size, output_size), cv2.INTER_CUBIC)
        ...
        if cfg.mask_mode >= 7 and cfg.mask_mode <= 9:
            ...
            dst_face_xseg_bgr = cv2.warpAffine(
                img_bgr, xseg_mat, (xseg_input_size,)*2, flags=cv2.INTER_CUBIC)
            dst_face_xseg_mask = xseg_256_extract_func(dst_face_xseg_bgr)
            X_dst_face_mask_a_0 = cv2.resize(
                dst_face_xseg_mask, (output_size, output_size), cv2.INTER_CUBIC)
        ...
        if cfg.mask_mode == 6:  # 'XSeg-prd'
            ...
            wrk_face_mask_a_0 = prd_face_mask_a_0 * prd_face_dst_mask_a_0 * \
                X_prd_face_mask_a_0 * X_dst_face_mask_a_0
    ...
    if wrk_face_mask_a_0.shape[0] != mask_subres_size:
        wrk_face_mask_a_0 = cv2.resize(
            wrk_face_mask_a_0, (mask_subres_size, mask_subres_size), cv2.INTER_CUBIC)
    ...
    if 'raw' not in cfg.mode:
        ...
        if ero > 0:
            wrk_face_mask_a_0 = cv2.erode(wrk_face_mask_a_0, cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE, (ero, ero)), iterations=1)
        elif ero < 0:
            wrk_face_mask_a_0 = cv2.dilate(wrk_face_mask_a_0, cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE, (-ero, -ero)), iterations=1)
        ...
        if blur > 0:
            blur = blur + (1-blur % 2)
            wrk_face_mask_a_0 = cv2.GaussianBlur(
                wrk_face_mask_a_0, (blur, blur), 0)
    ...
    img_face_mask_a = cv2.warpAffine(wrk_face_mask_a_0, face_mask_output_mat, img_size, np.zeros(
        img_bgr.shape[0:2], dtype=np.float32), flags=cv2.WARP_INVERSE_MAP | cv2.INTER_CUBIC)[..., None]
    ...
    if wrk_face_mask_a_0.shape[0] != output_size:
        wrk_face_mask_a_0 = cv2.resize(
            wrk_face_mask_a_0, (output_size, output_size), cv2.INTER_CUBIC)
    ...
    if cfg.mode == 'original':
        ...
        if cfg.mode == 'raw-rgb':
            out_img = cv2.warpAffine(prd_face_bgr, face_output_mat, img_size, img_bgr.copy(
            ...
            if min(lenx, leny) >= 4:
                ...
                if 'seamless' not in cfg.mode and cfg.color_transfer_mode != 0:
                    if cfg.color_transfer_mode == 1:  # rct
                        ...
                        prd_face_bgr = np.clip(prd_face_bgr.astype(
                            np.float32) / 255.0, 0.0, 1.0)
                        ...
                        prd_face_bgr = imagelib.color_transfer_mkl(
                            prd_face_bgr*wrk_face_mask_area_a, dst_face_bgr*wrk_face_mask_area_a)
                        ...
                        prd_face_bgr = imagelib.color_transfer_sot(
                            prd_face_bgr*wrk_face_mask_area_a, dst_face_bgr*wrk_face_mask_area_a, steps=10, batch_size=30)
                ...
                if cfg.mode == 'hist-match':
                    ...
                    prd_face_bgr = imagelib.color_hist_match(
                        hist_match_1, hist_match_2, cfg.hist_match_threshold).astype(dtype=np.float32)
                ...
                out_img = cv2.warpAffine(prd_face_bgr, face_output_mat, img_size, np.empty_like(
                    img_bgr), cv2.WARP_INVERSE_MAP | cv2.INTER_CUBIC, cv2.BORDER_TRANSPARENT)
                out_img = np.clip(out_img, 0.0, 1.0)
                ...
                if 'seamless' in cfg.mode:
                    try:
                        ...
                        if 'MemoryError' in e_str:
                            ...
                            raise Exception("Seamless fail: " + e_str)
                        else:
                            print("Seamless fail: " + e_str)

                cfg_mp = cfg.motion_blur_power / 100.0

                out_img = img_bgr*(1-img_face_mask_a) + \
                    (out_img*img_face_mask_a)
                ...
                if ('seamless' in cfg.mode and cfg.color_transfer_mode != 0) or \
                   ...
                   cfg.bicubic_degrade_power != 0:
                    ...
                    out_face_bgr = cv2.warpAffine(
                        out_img, face_mat, (output_size, output_size), flags=cv2.INTER_CUBIC)
                    ...
                    if 'seamless' in cfg.mode and cfg.color_transfer_mode != 0:
                        if cfg.color_transfer_mode == 1:
                            out_face_bgr = imagelib.reinhard_color_transfer(np.clip(out_face_bgr*wrk_face_mask_area_a*255, 0, 255).astype(np.uint8),
                                                                            np.clip(dst_face_bgr*wrk_face_mask_area_a*255, 0, 255).astype(np.uint8))
                            out_face_bgr = np.clip(out_face_bgr.astype(
                                np.float32) / 255.0, 0.0, 1.0)
                        elif cfg.color_transfer_mode == 2:  # lct
                            ...
                            out_face_bgr = imagelib.color_transfer_mkl(
                                out_face_bgr*wrk_face_mask_area_a, dst_face_bgr*wrk_face_mask_area_a)
                        elif cfg.color_transfer_mode == 5:  # idt
                            ...
                            out_face_bgr = imagelib.color_transfer_sot(
                                out_face_bgr*wrk_face_mask_area_a, dst_face_bgr*wrk_face_mask_area_a, steps=10, batch_size=30)
                    ...
                    if cfg.blursharpen_amount != 0:
                        out_face_bgr = imagelib.blursharpen(
                            out_face_bgr, cfg.sharpen_mode, 3, cfg.blursharpen_amount)
                    ...
                    if cfg.bicubic_degrade_power != 0:
                        ...
                        img_bgr_downscaled = cv2.resize(
                            img_bgr, (int(img_size[0]*p), int(img_size[1]*p)), cv2.INTER_CUBIC)
                        img_bgr = cv2.resize(
                            img_bgr_downscaled, img_size, cv2.INTER_CUBIC)

                    new_out = cv2.warpAffine(out_face_bgr, face_mat, img_size, np.empty_like(
                        img_bgr), cv2.WARP_INVERSE_MAP | cv2.INTER_CUBIC, cv2.BORDER_TRANSPARENT)

                    out_img = np.clip(
                        img_bgr*(1-img_face_mask_a) + (new_out*img_face_mask_a), 0, 1.0)
...
def MergeMaskedFace2Face(predictor_func,
                         predictor_input_shape,
                         face_enhancer_func,
                         xseg_256_extract_func,
                         cfg,
                         frame_info_expression=None, frame_info_bgr=None):
    # 1.prepare background
    img_bgr_uint8 = frame_info_bgr.frame.rgb
    img_bgr_uint8 = imagelib.normalize_channels(img_bgr_uint8, 3)
    img_bgr = img_bgr_uint8.astype(np.float32) / 255.0

    # 2.prepare expression face
    expression_face = frame_info_expression.final_faces[0]
    expression_face = imagelib.normalize_channels(expression_face, 3)
    expression_face = expression_face.astype(np.float32) / 255.0
    expression_face = np.clip(expression_face, 0, 1)
    outs = []
    for face_num, img_landmarks in enumerate(frame_info_bgr.landmarks):
        out_img, out_img_merging_mask = MergeMaskedFace(
            predictor_func, predictor_input_shape, face_enhancer_func, xseg_256_extract_func, cfg, frame_info_bgr, img_bgr_uint8, img_bgr, img_landmarks, expression_face=expression_face)
